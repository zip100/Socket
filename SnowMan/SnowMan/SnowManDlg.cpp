...
void CSnowManDlg::initMessageList() 
{
	...
	// 为列表视图控件添加三列   
	message_list.InsertColumn(0, _T("类型"), LVCFMT_CENTER, rect.Width() / 5, 0);
	message_list.InsertColumn(1, _T("时间"), LVCFMT_CENTER, rect.Width() / 5, 1);
	message_list.InsertColumn(2, _T("内容"), LVCFMT_CENTER, rect.Width() / 5 * 3, 2);

	InsertMessage(_T("系统消息"), _T("初始化成功"));
}
